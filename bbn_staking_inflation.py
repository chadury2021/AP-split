...
class StakingSimulator:
    def __init__(
        ...
        self.BBNPrice = self.FDV / self.TotalBBN
    ...
    def calculate_apr_without_s4(self):
        total_apr_without_s4 = (
            self.btc_staking_apr + self.lrt2_rewards_apr + self.collateral_yield
        )
    ...
    def calculate_leveraged_apr_without_s4(self):
        leveraged_apr_without_s4 = (
            self.calculate_net_apr_without_s4() * self.max_leverage
        )
...
class StreamlitUI(SimulatorUI):
    ...
    def get_inputs(self) -> SimulatorInputs:
        ...
        TVL = st.sidebar.number_input(
            "Total Value Locked (TVL) in USD", value=10000000000
        )
        FDV = st.sidebar.number_input(
            "Fully Diluted Value (FDV) in USD", value=4000000000
        )
        StakingRatio = st.sidebar.slider(
            "Staking Ratio", min_value=0.0, max_value=1.0, value=0.4
        )
        TotalBBN = st.sidebar.number_input(
            "Total supply of BBN tokens", value=10000000000
        )
        InflationRate = st.sidebar.slider(
            "Annual Inflation Rate (%)", min_value=0.0, max_value=100.0, value=8.0
        )
        BTCSplit = st.sidebar.slider(
            "BTC Split (%)", min_value=0.0, max_value=100.0, value=50.0
        )
        st.sidebar.write(f"BBN Split: {100 - BTCSplit}%")
        BBNSplit = 100 - BTCSplit

        btc_staking_apr = st.sidebar.number_input(
            "BTC Staking APR (%)", min_value=0.0, value=1.6, step=0.01
        )
        lrt2_rewards_apr = st.sidebar.number_input(
            "LRT2 Rewards APR (%)", min_value=0.0, value=0.6, step=0.01
        )
        estimated_s4_added_apr = st.sidebar.number_input(
            "Estimated S4 Added APR (%)", min_value=0.0, value=0.5, step=0.01
        )
        collateral_yield = st.sidebar.number_input(
            "Collateral Yield (%)", min_value=0.0, value=0.01, step=0.01
        )
        borrow_cost = st.sidebar.number_input(
            "Borrow Cost (%)", min_value=0.0, value=1.0, step=0.01
        )
        ltv_ratio = st.sidebar.slider(
        ...
        max_leverage = st.sidebar.number_input(
            "Max Leverage", min_value=1.0, value=4.0, step=0.1
        )
    ...
    def display_results(
        ...
        btc_apy: float,
        bbn_apy: float,
        apr_without_s4: float,
        net_apr_without_s4: float,
        leveraged_apr_without_s4: float,
    ):
        st.header("Results")
...
if __name__ == "__main__":
    ...
    simulator = StakingSimulator(
        ...
        inputs.BTCSplit,
        inputs.BBNSplit,
        inputs.btc_staking_apr,
        inputs.lrt2_rewards_apr,
        inputs.estimated_s4_added_apr,
        inputs.collateral_yield,
        inputs.borrow_cost,
        inputs.ltv_ratio,
        inputs.max_leverage,
    )
    ...
    ui.display_results(
        btc_apy, bbn_apy, apr_without_s4, net_apr_without_s4, leveraged_apr_without_s4
    )
